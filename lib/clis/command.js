...
class CommandSet {
  ...
  showHelpInfo(){
    console.log('分布式路由前端项目初始化工具  recd-cli')
    console.log()
    console.log('     Examples: ecd-cli init myApp(省略时，则为当前所在目录文件夹名称)');
    console.log('     执行recd-cli -h查看更多')
  }
  command(){
    const self = this;
  ...
  async generateTemplate(){
    ...
    try {
      ...
      if(self.config.defineAppName){
        ...
        fs.writeFileSync(`${self.baseDir}/${self.config.appName}/package.json`, JSON.stringify(newPackageJson));
      }else{
        fs.writeFileSync(`${self.baseDir}/package.json`, JSON.stringify(newPackageJson));
      }
